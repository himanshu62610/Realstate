...
function testfixer (filename) {
  ...
  return through(function (line, enc, cb) {
    ...
    line = line.replace(/(.*console\..*)/g, '

    
    if (filename === 'test-buffer-big.js') {
      line = line.replace(/(.*new Int8Array.*RangeError.*)/, '
      line = line.replace(/(.*new ArrayBuffer.*RangeError.*)/, '
      line = line.replace(/(.*new Float64Array.*RangeError.*)/, '
    }
    ...
    if (filename === 'test-buffer.js') {
      line = line.replace(/^(\s*)(var crypto = require.*)/, '$1
