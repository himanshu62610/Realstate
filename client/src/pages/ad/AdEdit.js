...
import AddressAutocomplete from "../../components/forms/address"
...
export default function AdEdit({ action, type }) {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const { data } = await axios.post("http://localhost:8000/adapi/ad/${ad._id}",ad);
...
export default function AdEdit({ action, type }) {
  ...
  const fetchAd = async () => {
    try {
      const { data } = await axios.get(`http://localhost:8000/adapi/ad/${params.slug}`);
  ...
  const handleClick = async () => {
    try {
      ...
      if (!ad.photos?.length) {
        ...
        const { data } = await axios.put(`http://localhost:8000/adapi/ad/${ad._id}`, ad);
        // console.log("ad create response => ", data);
        if (data?.error) {
          toast.error(data.error);
          setAd({ ...ad, loading: false });
  ...
  const handleDelete = async () => {
    try {
      ...
      const { data } = await axios.delete(`http://localhost:8000/adapi/ad/${ad._id}`);
      // console.log("ad create response => ", data);
      if (data?.error) {
        toast.error(data.error);
        setAd({ ...ad, loading: false });
